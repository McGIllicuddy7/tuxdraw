...
void *array_concat(void *array, void *target, size_t object_size,
                   size_t addr_size);
...
#define resize(target, new_size)                                               \
  target = array_resize(target, new_size, sizeof(*target))
#define append(target, addr)                                                   \
  resize(target, len(target) + 1);                                             \
  target[len(target) - 1] = addr

#define concat(target, addr)                                                   \
  target = array_concat(target, addr, sizeof(*target), sizeof(*addr))
...
#define insert(target, idx, value)                                             \
  ...
  memmove(&target[idx + 1], &target[idx],                                      \
          (len(target) - idx - 1) * sizeof(*target));                          \
  target[idx] = value
...
#define enable_hash_type(T, U)                                                 \
  ...
  static T##U##HashTable *T##U##HashTable_create(                              \
      size_t size, size_t (*hash_func)(T), bool (*eq_func)(T, T)) {            \
  ...
  static void T##U##HashTable_resize(T##U##HashTable *table,                   \
                                     size_t new_size) {                        \
    ...
    for (int i = 0; i < table->TableSize; i++) {                               \
      for (int j = 0; j < len(table->Table[i]); j++) {                         \
        ...
        T##U##KeyValuePair pair = {.key = table->Table[i][j].key,              \
                                   .value = table->Table[i][j].value};         \
  ...
  static T##U##KeyValuePair *T##U##HashTable_find_kv(T##U##HashTable *table,   \
                                                     T key) {                  \
    size_t hashval = table->hash_func(key);                                    \
    size_t hash = hashval % table->TableSize;                                  \
    for (int i = 0; i < len(table->Table[hash]); i++) {                        \
  ...
  static void T##U##HashTable_insert(T##U##HashTable *table, T key, U value) { \
    ...
    T##U##KeyValuePair pair =                                                  \
        (T##U##KeyValuePair){.key = key, .value = value};                      \
